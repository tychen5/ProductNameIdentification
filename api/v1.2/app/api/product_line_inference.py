...
final_product_df = pd.read_pickle(data_path + 'your_final_product_df.pkl')
fl_convert_dict = pickle.load(open(data_path + 'your_fl_mapping.pkl', 'rb'))
new_em_df = pd.read_pickle(data_path + 'your_em2_detect_df.pkl')
en_vocabulary, stop_words = pickle.load(open(data_path + 'your_envocab_stopword.pkl', 'rb'))
detect_di_title = pickle.load(open(data_path + 'your_reverse_pn2pl_di_lowercase.pkl', 'rb'))
all_names_title = list(detect_di_title.keys())
detect_di_selftext = pickle.load(open(data_path + 'your_reverse_pn2pl_di.pkl', 'rb'))
all_names_selftext = list(detect_di_selftext.keys())
classifier = pickle.load(open(data_path + 'your_zslmodel.pkl', 'rb'))
...
def get_model_res(title_res, thr, detect_di):
    ...
    Args:
        title_res (DataFrame): Results DataFrame.
        thr (float): Threshold for filtering results.
        detect_di (dict): Dictionary for label conversion.

    Returns:
        list: Ordered list of model results.
    """
...
def get_intersection(li1, li2, default_order=False):
    """
    Get the intersection of two lists while keeping the order of the first list.

    Args:
        li1 (list): First list.
        li2 (list): Second list.
        default_order (bool, optional): Use default order if True. Defaults to False.
