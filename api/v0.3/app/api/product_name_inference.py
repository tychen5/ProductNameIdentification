...
def easy_cleansing_doc(doc):
    '''
    Perform basic text cleaning on the input document.
    '''
...
def calc_doc_score(sent_kw,reddit_text):
    ...
    score = 0
    for kw in sent_kw:
        s = max(fuzz.token_set_ratio(reddit_text,kw),fuzz.token_set_ratio(kw,reddit_text))/100 # Calculate the token set ratio between the Reddit post and the keyword
        if s>=0.8:
            if ' ' in kw:
                score = score + 1/2*(s) # If the keyword contains a space, add half of the score to the total
            else:
                if (kw not in stop_words)and(kw not in en_vocabulary):
                    score = score + 1/3*(s) # If the keyword is not in the stop words or English vocabulary, add one third of the score to the total
                elif kw not in stop_words:
                    score = score + 1/4*(s) # If the keyword is not in the stop words, add one fourth of the score to the total
                else:
                    score = score + 1/5*(s) # Otherwise, add one fifth of the score to the total
    return score
...
def combine_store_names(sn_li):
    ...
    if len(sn_li)==1:
        ...
        sn_li.sort(key=lambda s:len(s),reverse=True) # Sort the list of store names by length in descending order
        for i,name in enumerate(sn_li): # Iterate through the list of store names
            if i==0:
                l=name.lower().split()
                r=sn_li[i+1].lower().split()
                merged = merge(l,r) # Merge the first two store names
                merged_txt = ' '.join(' '.join(x) for x in merged)
            else:
                l = merged_txt.split()
                r = sn_li[i+1].lower().split()
                merged = merge(l,r) # Merge the current store name with the previous merged store name
                merged_txt = ' '.join(' '.join(x) for x in merged)                
            if i+2==len(sn_li):
                return merged_txt
                break
...
def merge_diff_sn(li_str):
    ...
    if len(li_str)==1:
        ...
        all_together = [] # List of lists of store names
        taken_idx = []
        for i,left in enumerate(li_str):
            ...
            for j,right in enumerate(li_str):
                if (j in taken_idx) or (j<=i):
                    ...
                    if s.ratio()>0.8: # If the similarity ratio is greater than 0.8, add the store name to the current list
        ...
        for ali in all_together:
            taken_li.append(combine_store_names(ali)) # Combine the store names in each list
        return taken_li
...
def get_product_name_doc(reddit_doc,product_df=final_product_df,thr=0.95):
    '''
    Get the product name, first level, second level, and third level categories for a given Reddit post.
    '''
    ...
    try:
        ...
        merged_sn_li.sort(key=lambda s:len(s),reverse=True) # Sort the list of merged store names by length in descending order
...
def product_name_inference_inhouse(input_post):
    '''
    Perform product name inference on a given Reddit post.
    '''
