...
def calc_doc_score(sent_kw, reddit_text):
    ...
    for kw in sent_kw:
        ...
        if s >= 0.8:
            if ' ' in kw:
                # If the keyword contains a space, it is considered strong
                score = score + 1 / 1.8 * (s)
            else:
                if (kw not in stop_words) and (kw not in en_vocabulary):
                    # If the keyword is not a stop word or in the English vocabulary, it is considered strong
                    score = score + 1 / 3 * (s)
                elif kw not in stop_words:
                    # If the keyword is not a stop word, it is considered medium
                    score = score + 1 / 3.5 * (s)
                else:
                    # If the keyword is a stop word, it is considered weak
                    score = score + 1 / 4 * (s)
    return score
...
def merge_diff_sn(li_str):
    '''
    Merge store names using the difflib library and a similarity threshold.
    '''
    if len(li_str) == 1:
        return li_str[0]
    else:
...
def get_product_name_doc(reddit_doc, product_df=final_product_df, thr=0.85):
    '''
    ...
    # Filter the product dataframe by score threshold
