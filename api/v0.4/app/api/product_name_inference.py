...
def get_product_name_doc(reddit_doc,product_df=final_product_df,thr=0.95):
    '''
    Get the product name from the Reddit document.
    '''
    ...
    try:
        ...
        merged_sn_li.sort(key=lambda s:len(s),reverse=True) # Sort the list of merged store names by length, from longest to shortest
